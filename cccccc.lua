...
-- LICENSE GATE (Ed25519 + heartbeat key.chuivn.com) ĐÃ GỠ BỎ HẲN theo yêu cầu chồng — KHÔNG còn check key.
...
-- ============================================================
-- DEEP RAM PURGE (dot 13 - "toi uu RAM cuc doan", DO THAT qua bridge 2026-07-18):
-- MemWatch chi do heap LUA; RAM process cao chu yeu la ENGINE (texture/sound/particle cua CAY/QUA
-- MOI MOC sau init + vuon nguoi khac). Strip 1-lan luc init KHONG bat cay moc sau -> RAM leo dan
-- (do: Total 2125->2596MB trong vai phut). DeepRamPurge do RAM ENGINE that (Stats:GetTotalMemoryUsageMb)
-- moi Delay giay; vuot EngineRamMB -> strip lai texture/particle/sound toan workspace + don vuon
-- NGUOI KHAC + GC. DO BRIDGE 1 luot: sound -74MB, particle -37MB, tong ~-100MB.
-- Viet tat: DeepRamPurge = true/false hoac = <so> (so = EngineRamMB nguong).
if type(CFG.DeepRamPurge) == "boolean" then
    CFG.DeepRamPurge = { Enabled = CFG.DeepRamPurge }
elseif type(CFG.DeepRamPurge) == "number" then
    CFG.DeepRamPurge = { Enabled = true, EngineRamMB = CFG.DeepRamPurge }
end
CFG.DeepRamPurge = CFG.DeepRamPurge or {}
setDefault(CFG.DeepRamPurge, "Enabled", false)
setDefault(CFG.DeepRamPurge, "EngineRamMB", 2300)      -- nguong RAM engine (MB) kich purge sau; <nguong = chi do, khong lam gi
setDefault(CFG.DeepRamPurge, "Delay", 45)              -- giay giua 2 lan do RAM engine
setDefault(CFG.DeepRamPurge, "StripTextures", true)    -- MeshPart/SpecialMesh TextureID="", destroy SurfaceAppearance/Decal/Texture (chi visual)
setDefault(CFG.DeepRamPurge, "StripParticles", true)   -- destroy ParticleEmitter/Trail/Beam/Smoke/Fire/Sparkles
setDefault(CFG.DeepRamPurge, "StripSounds", true)      -- destroy Sound (tru GrowSFX - PlantVisualizer tham chieu)
setDefault(CFG.DeepRamPurge, "PurgeOtherGardens", true)-- don Plants/Visual/Sprinklers/Signs cua plot != minh (bot khong hai vuon nguoi khac)
setDefault(CFG.DeepRamPurge, "BatchSize", 3000)        -- nha frame moi N instance khi duyet (chong do)
...
-- DỌN CÂY RÁC (đợt 11 - PurgeUnlisted): MODE RIÊNG, độc lập với TrimToQuota. Bật = quét vườn,
-- cây KHÔNG có trong PlanQuota bị đào SẠCH NGAY (kể cả mutation Gold/Rainbow/Mega), KHÔNG chờ
-- gate Limit/DestroyUntil. Tắt (false) = không làm gì. Viết tắt: PurgeUnlisted = true/false.
if type(CFG.PurgeUnlisted) == "boolean" then
    CFG.PurgeUnlisted = { Enabled = CFG.PurgeUnlisted }
end
CFG.PurgeUnlisted = CFG.PurgeUnlisted or {}
setDefault(CFG.PurgeUnlisted, "Enabled", false)   -- mặc định TẮT (đào là mất vĩnh viễn)
setDefault(CFG.PurgeUnlisted, "Delay", 60)        -- giây giữa 2 lần quét (đã dọn sạch thì quét rỗng, rất nhẹ)
setDefault(CFG.PurgeUnlisted, "DigDelay", 0.1)    -- nghỉ giữa mỗi nhát đào (đừng < 0.05 dễ bị server chặn)
...
setDefault(CFG.HideTreeKeepFruit, "Delay", 1)
...
-- ket vinh vien -> RAM TANG dan theo so cay/qua moi. Qua nguong nay (giay) van chua co tag -> don (fallback).
...
-- MẶC ĐỊNH TẮT vì RẤT mạnh tay (xóa Baseplate/NPCS/map -> không hồi lại, ship cho khách phải cẩn thận).
-- An toàn đã kiểm: bán hàng dùng remote Networking.NPCS.SellAll (bắn từ xa được) -> xóa workspace.NPCS
-- KHÔNG hỏng AutoSell. Plot mình LUÔN giữ. Baseplate chỉ xóa khi đang ĐỨNG TRÊN plot (tránh rớt void).
...
--   Lighting: RainEffect/LightningEffect/RainbowEffect/StarEffect + ActiveNightAtmosphere. Đều VISUAL -> xoá an toàn.
...
local function actionLog(action, state, ...)
    ...
    pushActionLine(line)   -- LUON cap nhat GUI status (State.ActionLines) du Debug=false.
...
-- BOUND WebhookSeen (chong leak RAM lau dai khi farm 24/7): key dedup (seed:/gold:/petbuy:) la duy nhat
    ...
    for _, tool in ipairs(seeds) do
        ...
        elseif onlyPlant and not nameMatchesConfiguredSet(seedName, onlyPlant) then
            ...
            if useQuota and type(seedName) == "string" and not freshFill
                and not singleBypass and not singleFill then
...
function Runtime.doAutoShovelReplace()
    ...
    for _, plant in ipairs(plantModels) do
        ...
        if not (type(mut) == "string" and keepMut[mut]) and not fillSingle then
...
-- ============================================================
-- DỌN CÂY RÁC (đợt 11 - PurgeUnlisted): mode RIÊNG bật/tắt bằng CFG.PurgeUnlisted (true/false).
-- Bật -> đọc vườn, cây có SeedName KHÔNG nằm trong PlanQuota đào HẾT NGAY, bất kể mutation
-- (không có ngoại lệ KeepMutations như TrimToQuota), KHÔNG chờ gate Limit/DestroyUntil.
-- Dùng Y HỆT cơ chế đào đã chạy ổn của TrimToQuota: Shovel.UseShovel(plant.Name, "", shovelAttr, shovel).
-- Không đụng logic TrimToQuota: 2 mode chạy song song, purge tự nhường khi trim đang đào (TrimActiveUntil).
-- ============================================================
function Runtime.doPurgeUnlisted()
    local c = CFG.PurgeUnlisted
    if not (c and c.Enabled) then return end
    if not Runtime.FarmGateReady("PurgeUnlisted") then return end   -- chờ fps boot + DATA vườn về đủ
    if State.AntiStealEngaging or State.SellInProgress then return end
    if Runtime.ShouldYieldForSeedPriority and Runtime.ShouldYieldForSeedPriority("PurgeUnlisted") then return end
    -- TrimToQuota đang cầm xẻng đào dở -> nhường vòng này (tránh 2 task giành ToolLock/equip)
    if (tonumber(State.TrimActiveUntil) or 0) > os.clock() then return end
    if not packet({ "Shovel", "UseShovel" }) then
        logw("PurgeUnlisted: thieu Networking.Shovel.UseShovel -> tat.")
        c.Enabled = false
        return
    end

    -- Nguồn quota GIỐNG HỆT TrimToQuota: PlanQuota -> TrimToQuota.Quota -> AutoPlant.PlantQuota.
    local quota = (type(CFG.PlanQuota) == "table" and next(CFG.PlanQuota) and CFG.PlanQuota)
        or (CFG.TrimToQuota and type(CFG.TrimToQuota.Quota) == "table" and next(CFG.TrimToQuota.Quota) and CFG.TrimToQuota.Quota)
        or (CFG.AutoPlant and type(CFG.AutoPlant.PlantQuota) == "table" and CFG.AutoPlant.PlantQuota)
    if not (quota and next(quota)) then
        actionLog("PurgeUnlisted", "SKIP", "khong co PlanQuota")
        return
    end
    ...
    -- Gom cây RÁC: tra attribute SeedName (giống TrimToQuota) -> không có key trong quota = đào.
    -- Mutation KHÔNG cứu (ý chồng: "đào hết dù mutation gì").
    local junk = {}
    for _, m in ipairs(plantsFolder:GetChildren()) do
        if m:IsA("Model") then
            local sName = m:GetAttribute("SeedName")
            if type(sName) == "string" and sName ~= "" and quota[sName] == nil then
                junk[#junk + 1] = m
            end
        end
    end
    if #junk == 0 then return end   -- vườn sạch -> vòng quét rỗng, không tốn gì thêm

    unequipSeedTool("PurgeUnlisted")
    local shovel = getShovelTool()
    if not shovel then
        actionLog("PurgeUnlisted", "SKIP", "missing shovel tool")
        return
    end
    local shovelAttr = shovel:GetAttribute("Shovel")
    Runtime.TryToolLock("PurgeUnlisted", 8, true)

    local digDelay = math.max(tonumber(c.DigDelay) or 0.1, 0)
    State.TrimActiveUntil = os.clock() + 15   -- báo "đang đào" -> tác vụ phụ nhường y như khi trim
    local dug = 0
    for _, plant in ipairs(junk) do
        -- EVENT rainbow/gold seed nổ -> DỪNG đào ngay đi claim (event ưu tiên nhất, giống trim)
        if Runtime.ShouldYieldForSeedPriority and Runtime.ShouldYieldForSeedPriority("PurgeUnlisted") then break end
        if plant.Parent then
            if not equipTool(shovel) then
                actionLog("PurgeUnlisted", "SKIP", "cannot equip shovel")
                break
            end
            -- fruitId="" -> đào cả cây (y hệt TrimToQuota)
            local ok = firePacket({ "Shovel", "UseShovel" }, plant.Name, "", shovelAttr, shovel)
            Runtime.ExtendToolLock("PurgeUnlisted", 5)
            if ok then
                dug = dug + 1
                State.TrimActiveUntil = os.clock() + 15   -- refresh: vẫn đang dọn
                actionLog("PurgeUnlisted", "DIG", tostring(plant:GetAttribute("SeedName")))
            end
            if not waitAlive(digDelay) then State.TrimActiveUntil = 0; return end
        end
    end
    State.TrimActiveUntil = 0
    Runtime.ReleaseToolLock("PurgeUnlisted")
    if dug > 0 then
        Runtime.ClearPlotFull()   -- còn chỗ trống -> AutoPlant trồng cây trong plan vào
        actionLog("PurgeUnlisted", "DONE", ("dug=%d rac"):format(dug))
    end
end
...
local function getOwnedCrateOrPropCount(crateName)
...
function Runtime.doAutoBuyCrate()
    ...
    local function buyCrate(crateName, price, targetCount)
        ...
        targetCount = tonumber(targetCount) or 0
        if targetCount > 0 then
            local ownedCount = getOwnedCrateOrPropCount(crateName)
            if ownedCount >= targetCount then
...
function Runtime.SetupGardenTracker()
    ...
    connect(sync, function(all)
        ...
        for plantId in pairs(tracked) do
            if not fresh[plantId] then tracked[plantId] = nil end
        end
    ...
    connect(pRem, function(uid, plantId) if isMine(uid) and plantId then tracked[plantId] = nil end end)
...
function Runtime.ScanHarvestTargets(ripeOnly, statsOnly)
    ...
    local totalF, ripeF, stuck, nSingle = 0, 0, 0, 0
        ...
        if type(pl) == "table" and pl.IsPotted ~= true then  -- PickFruit source bo qua cay trong chau
            ...
            if Runtime.IsSingleHarvestSeed(pl.PlantName) then
                ...
                if ma > 0 and (tonumber(pl.Age) or 0) >= ma then
                    ...
                    nSingle = nSingle + 1
                    if singles then
                        -- entry pool {P=plantId, R=do hiem} (truoc la chuoi tran; doi sang bang de sort theo R)
                ...
                for fruitId, f in pairs(fr) do
                    if type(f) == "table" then
                        ...
                        if ripeOnly then
                            ...
                            ripe = (ma <= 0) or (Runtime.EstFruitAge(key, f, now) >= ma)
                        end
                        if ripe then
                            ripeF = ripeF + 1
                            if fruits then
...
function Runtime.doDataHarvest(c)
    ...
    -- UU TIEN HAI (chong chot): (1) cay 1-LAN-HAI truoc het (PrioritizeSingleHarvest mac dinh TRUE;
    -- dat false thi cay 1-lan xep SAU qua), (2) trong moi nhom sort DO HIEM GIAM DAN (SeedData.Rarity
    -- qua Runtime.SeedRarityScore) -> cay/qua hiem cao ban remote truoc. Sort tai cho tren buffer pool
    -- (khong cap phat bang moi - giu thanh qua RAM dot 3).
    table.sort(singles, function(a, b) return (a.R or 0) > (b.R or 0) end)
    table.sort(fruits, function(a, b) return (a.R or 0) > (b.R or 0) end)
...
function Runtime.doAutoCollectDropsBody()
                ...
                if label then
                    ...
                    if pos then
                        table.insert(items, {
                            ...
                            Gold = isGold,
                            Mega = isMega,
                            Event = isEvent,   -- Rainbow/Gold/Mega/seed dac biet moi -> ban dem van lay
                            Seed = true,
                        })
                    end
                end
    ...
    if c.StayHomeAtNight ~= false and isNight() then
        ...
        for _, e in ipairs(items) do
            if e.Event or e.Rainbow or e.Gold or e.Mega then table.insert(onlyEvent, e) end
        end
        items = onlyEvent
    end
    ...
    pcall(function()
        for _, d in ipairs(char:GetDescendants()) do
            if d:IsA("Accessory") or d:IsA("Shirt") or d:IsA("Pants")
                ...
                or d:IsA("Decal") or d:IsA("SurfaceAppearance") then
...
Runtime.SetupCharacterStrip = function()
    ...
    local c = CFG.FpsBoost
    if not (c and c.Enabled and c.StripOtherCharacters ~= false) then return end
    ...
    local function hookPlayer(plr)
        ...
        local conn = plr.CharacterAdded:Connect(function(ch)
            task.wait(1)   -- cho phu kien load xong roi strip 1 the (handler chay thread rieng, yield ok)
            Runtime.StripCharacterJunk(ch)
        end)
        table.insert(Runtime.Cleanups, function() pcall(function() conn:Disconnect() end) end)
    end
    ...
    actionLog("FpsBoost", "CHAR_STRIP", "lot phu kien nguoi khac ON")
end
...
do
    ...
    local ok = pcall(function()
        ...
        if p and p.OnClientEvent and type(p.OnClientEvent.Connect) == "function" then
            p.OnClientEvent:Connect(onSeedClaimed)
        else
...
do
    local ok = pcall(function()
        ...
        rep.OnClientEvent:Connect(function(plr, packId, packName, seedName)
            ...
            local fired = pcall(function() conf:Fire(packId, packName, seedName) end)
        ...
        pcall(function()
            ...
            for _, inst in ipairs(Lighting:GetDescendants()) do
                if inst:IsA("BloomEffect")
                    ...
                    if inst:IsA("Atmosphere") or inst:IsA("Clouds") or inst:IsA("Sky") then
                        pcall(function() inst:Destroy() end)
                    else
                        pcall(function() inst.Enabled = false end)
                    end
...
function Runtime.doHideTreeKeepFruit()
    ...
    -- Cay chua ready -> doi (toi da WaitReadySeconds phong attribute thieu o edge-case) roi moi don.
    Runtime.HideTreePendingAt = Runtime.HideTreePendingAt or setmetatable({}, { __mode = "k" })
    local pendingAt = Runtime.HideTreePendingAt
    local readyWait = tonumber(c.WaitReadySeconds) or 6
    ...
    for _, plant in ipairs(plants:GetChildren()) do
        if plant:IsA("Model") or plant:IsA("Folder") then
            ...
            if not done[plant] and plant:GetAttribute("PlantGrowthReady") ~= true then
                local first = pendingAt[plant]
                if not first then
                    pendingAt[plant] = os.clock()
                    notReady = true
                elseif (os.clock() - first) < readyWait then
                    notReady = true
                end
            end
            ...
            if fruits and onlyRipe then
                for _, fruit in ipairs(fruits:GetChildren()) do
                    if not fruit:FindFirstChild("HarvestPrompt", true) then
            ...
            -- tag doi toi WaitReadySeconds (pendingAt) roi moi destroy (phong tag khong bao gio den).
            if fruits and c.DestroyFruitToo == true then
                for _, fruit in ipairs(fruits:GetChildren()) do
                    ...
                    if not ripeToKill then
                        local first = pendingAt[fruit]
                        if not first then
                            pendingAt[fruit] = os.clock()
                        elseif (os.clock() - first) >= readyWait then
                            ripeToKill = true
                        end
                    end
    ...
    if affected > 0 then
        ...
        if affected >= 200 then pcall(function() collectgarbage("collect") end) end
    end
end
...
function Runtime.SetupMapCleanup()
    ...
    local function setupWeatherGuard()
        ...
        for _, child in ipairs(ws:GetChildren()) do
            if wsW[child.Name] then forceDestroy(child) end
        end
        if lighting then
            for _, child in ipairs(lighting:GetChildren()) do
                if lightW[child.Name] then forceDestroy(child) end
            end
        end
        ...
        if lighting then
            local lconn = lighting.ChildAdded:Connect(function(child)
                ...
                if lightW[child.Name] then
                    task.defer(function() if child and child.Parent == lighting then forceDestroy(child) end end)
                end
            end)
            table.insert(Runtime.Cleanups, function() pcall(function() lconn:Disconnect() end) end)
        end
        actionLog("MapCleanup", "WEATHER_GUARD", "xoa VFX thoi tiet/event ON")
    end
    ...
    if CFG["Nuke All"] == true and not Runtime.NukeGuardConnected then
        ...
        task.spawn(function()
            ...
            pcall(function() collectgarbage("collect") end)   -- vua xoa ca ngan instance -> GC nha RAM ngay
        end)
    end

    -- EVENT: có người MỚI vào server -> chờ plot họ stream/gán xong rồi xóa (KHÔNG scan định kỳ).
    local conn = Players.PlayerAdded:Connect(function(plr)
        if not (CFG.MapCleanup and CFG.MapCleanup.Enabled) then return end
        actionLog("MapCleanup", "JOIN", tostring(plr and plr.Name))
        task.wait(tonumber(c.JoinWait) or 3)  -- đợi plot của người mới hiện ra
        cleanOtherPlots()
    end)
    table.insert(Runtime.Cleanups, function() pcall(function() conn:Disconnect() end) end)
end
    ...
    task.spawn(function()
        ...
        if c.PurgeDeadGui ~= false then
            local keepGui = {
                ...
                BackpackGui = true, HUD = true, SecretDropLog = true,
            }
            ...
            if pg then
                for _, gui in ipairs(pg:GetChildren()) do
                    if gui:IsA("ScreenGui") and not keepGui[gui.Name] then
                        if pcall(function() gui:Destroy() end) then g1 = g1 + 1 end
                    end
                end
            end
            pcall(function()
...
Runtime.SetupCommercialGui = function()
    ...
    task.spawn(function()
        ...
        while gui.Parent do
            ...
            local rd = math.max(tonumber(CFG.Dashboard and CFG.Dashboard.RefreshRate) or 1, 1)
...
Runtime.SetupRuntimeWatchdog = function()
    local thread = task.spawn(function()
        while isAlive() do
            ...
            if (enabled or Runtime.DashboardGuiName() == "KaitunCommercial") and not guiAlive then
...
task.spawn(Runtime.BootStartup)
task.spawn(Runtime.BootSequencer)   -- dot 5: dieu phoi "chay tung flow" (tha task theo wave)
...
loopTask("PurgeUnlisted", Runtime.doPurgeUnlisted, function() return (CFG.PurgeUnlisted and CFG.PurgeUnlisted.Delay) or 60 end)
...
Runtime.DoMemJanitor = function()
    ...
    -- 7) goi GC full neu executor ho tro (Luau chuan chi co "count" -> pcall nuot loi, vo hai)
    pcall(function() collectgarbage("collect") end)
end
...
Runtime.GetLuaHeapKB = function()
    ...
    if type(kb) ~= "number" and type(gcinfo) == "function" then
...
-- ============================================================
-- DEEP RAM PURGE (dot 13): do RAM ENGINE that + strip cuc doan khi vuot nguong. Bo tro cho MemWatch
-- (MemWatch chi lo heap Lua). Reactive: chi lam viec khi RAM engine >= EngineRamMB -> re, an toan.
-- ============================================================
Runtime.GetEngineRamMB = function()
    local mb
    pcall(function() mb = game:GetService("Stats"):GetTotalMemoryUsageMb() end)
    return tonumber(mb)
end

-- Strip texture/particle/sound toan workspace+ReplicatedStorage+SoundService. CAY/QUA MOC SAU init
-- khong duoc strip 1-lan -> RAM leo; strip lai dinh ky bat het. CHI VISUAL: khong xoa cay/qua, khong
-- dung tag/attribute/logic nao. GrowSFX GIU (PlantVisualizerController tham chieu SoundService.SFX.GrowSFX).
Runtime.DeepStrip = function(c)
    local batch = math.max(tonumber(c.BatchSize) or 3000, 200)
    local nTex, nPart, nSnd = 0, 0, 0
    local seen = 0
    for _, root in ipairs({ workspace, ReplicatedStorage, game:GetService("SoundService") }) do
        local ok, desc = pcall(function() return root:GetDescendants() end)
        if ok then
            for _, inst in ipairs(desc) do
                pcall(function()
                    if c.StripTextures ~= false then
                        if inst:IsA("SurfaceAppearance") then inst:Destroy(); nTex = nTex + 1; return end
                        if inst:IsA("Decal") or inst:IsA("Texture") then inst:Destroy(); nTex = nTex + 1; return end
                        if inst:IsA("MeshPart") then
                            if inst.TextureID ~= "" then inst.TextureID = ""; nTex = nTex + 1 end
                        elseif inst:IsA("SpecialMesh") then
                            if inst.TextureId ~= "" then inst.TextureId = ""; nTex = nTex + 1 end
                        end
                    end
                    if c.StripParticles ~= false and (inst:IsA("ParticleEmitter") or inst:IsA("Trail")
                        or inst:IsA("Beam") or inst:IsA("Smoke") or inst:IsA("Fire") or inst:IsA("Sparkles")) then
                        inst:Destroy(); nPart = nPart + 1; return
                    end
                    if c.StripSounds ~= false and inst:IsA("Sound") and inst.Name ~= "GrowSFX" then
                        inst:Destroy(); nSnd = nSnd + 1; return
                    end
                end)
                seen = seen + 1
                if seen % batch == 0 then task.wait() end
            end
        end
    end
    return nTex, nPart, nSnd
end

-- Don Plants/Visual/Sprinklers/Signs cua plot NGUOI KHAC (bot chi hai vuon MINH -> vuon nguoi khac
-- la rac RAM thuan). GIU plot minh qua getPlot() (PlotId attr). CHUA biet plot minh -> KHONG dam don
-- (khoi xoa nham vuon minh). DO BRIDGE: -30k instance/luot.
Runtime.PurgeOtherGardens = function()
    local gardens = workspace:FindFirstChild("Gardens")
    ...
    for _, plot in ipairs(gardens:GetChildren()) do
        if plot ~= myPlot then
            for _, part in ipairs({ "Plants", "Visual", "Sprinklers", "Signs" }) do
                local child = plot:FindFirstChild(part)
                if child then
                    n = n + 1
...
Runtime.DoDeepRamPurge = function()
    local c = CFG.DeepRamPurge
    if not (c and c.Enabled) then return end
    if not Runtime.BootReleasedAll then return end   -- chua boot xong -> khong dung cay/qua dang init
    local mb = Runtime.GetEngineRamMB()
    if mb then State.EngineRamMB = mb end
    local threshold = tonumber(c.EngineRamMB) or 2300
    if not mb or mb < threshold then return end
    Runtime.BeginTurboFps("deep ram purge")   -- mo cap: duyet 180k instance vu roi khoa lai cap cu
    local nTex, nPart, nSnd = Runtime.DeepStrip(c)
    local nG = (c.PurgeOtherGardens ~= false) and Runtime.PurgeOtherGardens() or 0
    Runtime.EndTurboFps()
    pcall(function() collectgarbage("collect") end)
    local after = Runtime.GetEngineRamMB()
    if after then State.EngineRamMB = after end
    actionLog("DeepRam", "PURGE", ("%dMB->%dMB tex=%d part=%d snd=%d grd=%d"):format(
        math.floor(mb + 0.5), math.floor((after or mb) + 0.5), nTex, nPart, nSnd, nG))
end
loopTask("DeepRamPurge", Runtime.DoDeepRamPurge, function()
    return tonumber(CFG.DeepRamPurge and CFG.DeepRamPurge.Delay) or 45
end)

...
Runtime.GetPriorityWildPetCandidate = function()
    ...
    if type(c.OwnLimit) == "table" and next(c.OwnLimit) ~= nil then
        ...
        for kName in pairs(c.OwnLimit) do
            if type(kName) == "string" then nameSet[normPetKey(kName)] = true end
        end
    ...
    for _, ref in ipairs(folder:GetChildren()) do
        if ref:IsA("BasePart") then
            ...
            if type(petName) == "string" and petName ~= "" and ownerUserId ~= LocalPlayer.UserId and price <= budget then
                ...
                local allow
                if nameSet then
                    allow = nameSet[normPetKey(petName)] == true
                else
...
function Runtime.doAutoCollectDrops()
    ...
    if c.StayHomeAtNight ~= false and isNight() then
        ...
        for _, e in ipairs(items) do
            if e.Rainbow or e.Gold then table.insert(onlyEvent, e) end
        end
        items = onlyEvent
    end
...
function Runtime.SetupAntiPush()
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        -- đang chủ động đi làm việc -> KHÔNG ghì (để teleport claim seed / bán / đánh trộm)
        if State.SeedClaimInProgress or State.SellInProgress or State.AntiStealEngaging then return end
        local root = getRootPart()
...
do
    ...
    local function buildMailPetCandidates(c)
        -- PetNames: MẢNG {"Tên"} (ngưỡng 1) HOẶC MAP {Tên=ngưỡng} -> chỉ gửi khi SỐ pet đó >= ngưỡng.
        local wanted = {}
        local minCount = {}
        if type(c.PetNames) == "table" then
            for key, value in pairs(c.PetNames) do
                if type(value) == "string" and value ~= "" then
                    wanted[string.lower(value)] = true                 -- dạng mảng {"Tên"}
                elseif type(key) == "string" and value ~= false then
                    wanted[string.lower(key)] = true                   -- dạng map {Tên=ngưỡng}
                    if tonumber(value) then minCount[string.lower(key)] = tonumber(value) end
                end
            end
        end
        if next(wanted) == nil then return {} end
        -- gom pet CHƯA equip theo tên
        local byName = {}
        for _, e in ipairs(Runtime.GetPetInventoryEntries()) do
            if not e.Equipped
                ...
                table.insert(byName[k], e)
            end
        end
        local out = {}
        for k, group in pairs(byName) do
            if #group >= (tonumber(minCount[k]) or 1) then   -- đủ ngưỡng mới gửi cả nhóm
                for _, e in ipairs(group) do table.insert(out, e) end
            end
        end
        -- Xịn nhất gửi trước.
        table.sort(out, function(a, b)
...
local function buildWildPetCandidates(c)
    ...
    if type(c.OwnLimit) == "table" and next(c.OwnLimit) ~= nil then
        ...
        for kName, v in pairs(c.OwnLimit) do
            if type(kName) == "string" and tonumber(v) then capMap[normPetKey(kName)] = tonumber(v) end
        end
    end
    local nameFilter

...
-- CHI arm (=> chi gui webhook) khi THUC SU dang XAI KeepSeedForSell (chong yeu cau):
--   (1) KHONG fresh-acc (luc fresh-acc KeepSeedForSell TAT -> trong+hai binh thuong, khong phai "xai chuc nang");
--   (2) dang co DIEU KIEN NHAN = weather mutation active HOAC gia sell cua cay >= KeepSeedForSellMulti (x4).
-- Ban binh thuong (khong weather + khong x4) -> KHONG arm -> KHONG gui webhook.
Runtime.KSFArm = function(cropName)
    ...
    if Runtime.IsFreshAccPhase() then return end   -- fresh-acc: KeepSeedForSell tat -> khong bao
    ...
    if type(cropName) == "string" and cropName ~= "" then
        ...
        if type(m) == "number" and m > 0 then sellMultNow = m end
    end
    local priceOk = sellMultNow >= need
    if not (active or priceOk) then return end   -- khong weather + gia chua x4 -> ban thuong -> KHONG bao
    ...
    if not (s and s.Armed) then
        ...
        s = {
            Armed = true,
            -- co ca weather LAN x4 -> UU TIEN WEATHER (khop logic KeepSeedForSell: weather uu tien hon gia)
    ...
    if type(cropName) == "string" and cropName ~= "" then
        ...
        if sellMultNow > (tonumber(s.SellMult) or 0) then s.SellMult = sellMultNow end
    end
end

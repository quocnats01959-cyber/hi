...
do
    ...
    -- KeepCropWeather (top-level cho tiện): giữ crop tới khi có weather-mutation mong muốn mới hái.
    if type(CFG.KeepCropWeather) == "table" then
        ensure("AutoCollect").KeepCropWeather = CFG.KeepCropWeather
    end
...
-- KEEP CROP WEATHER (chồng): GIỮ crop trên cây (không hái dù đã chín) tới khi nó mang 1 trong các
-- weather-mutation mong muốn rồi mới hái. Map tên CÂY -> list tên MUTATION. Rỗng = hái bình thường.
--   KeepCropWeather = { ["Tomato"]={"Sunburst","Aurora"}, ["Bamboo"]={"Starstruck","Bloodlit"} }
-- Cây KHÔNG có trong map -> hái như thường. Chỉ áp cho DATA-HARVEST (đọc .Mutation từ TrackedPlants,
-- đã nối FruitMutationUpdated/PlantMutationUpdated). TÊN mutation phải khớp chuỗi .Mutation THẬT của
-- game (khớp kiểu "chứa"); tên weather hiển thị có thể KHÁC tên mutation lưu trong data.
setDefault(CFG.AutoCollect, "KeepCropWeather", {})
...
function Runtime.doAutoShovelReplace()
    ...
    for _, plant in ipairs(plantModels) do
        ...
        if not (type(mut) == "string" and keepMut[mut]) and not fillSingle
            and not Runtime.IsCropWeatherCrop(sName) then
...
function Runtime.doTrimToQuota()
    ...
    for sName, b in pairs(bySpecies) do
        ...
        -- KEEP CROP WEATHER: cay dang giu cho weather mutation -> KHONG dao (du ngoai PlanQuota / PurgeUnlisted).
        -- cap=nil + unlisted=false -> "if cap and b.count > cap" phia duoi bo qua -> khong dao loai nay.
        if Runtime.IsCropWeatherCrop(sName) then
...
-- KEEP CROP WEATHER (chong): GIU crop tren cay (khong hai du da chin) toi khi no mang 1 trong cac
-- weather-mutation mong muon roi moi hai. Map ten CROP -> list ten MUTATION. Vd:
--   KeepCropWeather = { ["Tomato"]={"Sunburst","Aurora"}, ["Bamboo"]={"Starstruck","Bloodlit"} }
-- LUU Y TEN: so khop voi chuoi .Mutation THAT trong data (dang "chua" - vd mutation "Aurora" khop
-- want "aurora"). Ten weather hien thi co the KHAC ten mutation ($.MutationData khong co trong dump)
-- -> dung dung ten mutation game luu (vd Blood Moon -> co the la "Bloodlit"). Tra ve:
--   true  = GIU (chin nhung mutation chua khop -> KHONG hai, de tren cay cho weather)
--   false = HAI binh thuong (crop khong co rule, HOAC da co mutation mong muon)
Runtime.CropWeatherHold = function(plantName, mutation)
    local map = CFG.AutoCollect and CFG.AutoCollect.KeepCropWeather
    if type(map) ~= "table" or next(map) == nil or type(plantName) ~= "string" then return false end
    local wants
    local pn = normalizeItemName(plantName)
    for k, v in pairs(map) do
        if type(v) == "table" and normalizeItemName(k) == pn then wants = v break end
    end
    if type(wants) ~= "table" or next(wants) == nil then return false end   -- crop khong co rule -> hai
    local ml = type(mutation) == "string" and string.lower(mutation) or ""
    if ml ~= "" then
        for _, w in ipairs(wants) do
            if type(w) == "string" and w ~= "" and ml:find(string.lower(w), 1, true) then
                return false   -- da co mutation mong muon -> HAI
            end
        end
    end
    return true   -- co rule ma mutation chua khop -> GIU tren cay
end

-- Cay CO trong KeepCropWeather (dang giu cho weather mutation) -> TrimToQuota/PurgeUnlisted KHONG duoc
-- dao (khong thi dao mat truoc khi ra mutation = hong feature). Tra ve true neu crop co rule.
Runtime.IsCropWeatherCrop = function(plantName)
    -- Cho phep tat bao ve bang KeepCropWeatherProtectFromTrim=false (khi do trim dao ca cay dang giu).
    if CFG.AutoCollect and CFG.AutoCollect.KeepCropWeatherProtectFromTrim == false then return false end
    local map = CFG.AutoCollect and CFG.AutoCollect.KeepCropWeather
    if type(map) ~= "table" or type(plantName) ~= "string" then return false end
    local pn = normalizeItemName(plantName)
    for k, v in pairs(map) do
        if type(v) == "table" and next(v) ~= nil and normalizeItemName(k) == pn then return true end
    end
    return false
end
...
function Runtime.ScanHarvestTargets(ripeOnly, statsOnly)
        ...
        if type(pl) == "table" and pl.IsPotted ~= true then  -- PickFruit source bo qua cay trong chau
            ...
            if Runtime.IsSingleHarvestSeed(pl.PlantName) then
                ...
                if ma > 0 and (tonumber(pl.Age) or 0) >= ma then
                    if Runtime.CropWeatherHold(pl.PlantName, pl.Mutation) then
                        -- KeepCropWeather: cay 1-lan da chin nhung mutation chua khop -> GIU (khong hai)
                        held = held + 1
                    else
            ...
            if type(fr) == "table" then
                for fruitId, f in pairs(fr) do
                    if type(f) == "table" then
                        ...
                        if ripe and Runtime.CropWeatherHold(pl.PlantName, f.Mutation) then
                            -- KeepCropWeather: qua CHIN nhung mutation chua khop -> GIU tren cay (khong hai),
                            -- cho weather mutation. OvertimeGrowth van cong don (de lau = nhieu qua hon khi hai).
                            held = held + 1
                        elseif ripe then
                            ripeF = ripeF + 1
                            if fruits then
    ...
    hs.Held = held   -- KeepCropWeather: so crop chin dang GIU cho weather mutation
    ...
    for prompt in pairs(Runtime.HarvestPrompts) do
        ...
        if prompt:IsDescendantOf(plot) then
            ...
            if model then
                ...
                -- KeepCropWeather (chong): prompt-path (khi KHONG Nuke) cung GIU crop toi khi co weather
                -- mutation. Ten cay lay tu TrackedPlants[plantId].PlantName (co san); mutation lay tu model.
                if mutOk then
                    ...
                    if cropName and Runtime.CropWeatherHold(cropName, model:GetAttribute("Mutation")) then
                        mutOk = false
                    end
                end
                if mutOk then
...
function Runtime.doAutoCollectDropsBody()
    ...
    if State.AntiStealEngaging or State.SellInProgress then Runtime.ClearDropScanRefs(); return end
